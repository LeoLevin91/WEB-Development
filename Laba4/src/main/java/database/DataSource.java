...
public class DataSource {
    ...
    public User getUser(String login, String password) throws SQLException, ClassNotFoundException {
        Connection connection = getConnection();
        PreparedStatement statement = connection.prepareStatement("select id, username from users where login=? and password=?");
        statement.setString(1, login);
        statement.setString(2, password);
        ResultSet rs = statement.executeQuery();
        rs.next();
        ArrayList <TrackList> trackLists = new ArrayList<>();
        User tempUser = new User(rs.getInt("id"), rs.getString("username"), login, password, trackLists);
        trackLists = findTrackListByUser(tempUser);
